...
test('Rapidly click the "Add to Cart" button multiple times', async ({ page }) => {
  await page.goto('http://localhost/index.php');
  await page.getByRole('link', { name: ' My Account ' }).click();
  await page.getByRole('link', { name: 'Login' }).click();
  await page.getByRole('textbox', { name: 'E-Mail Address' }).click();
  ...
  await expect(page.getByText('Success: You have added').last()).toBeVisible();
  await page.getByRole('link', { name: ' Shopping Cart' }).click();
  await expect(page.getByRole('heading', { name: 'Shopping Cart' })).toBeVisible();
  await expect(page.locator('#output-cart').getByText('Samsung SyncMaster 941BW')).toBeVisible();
});
...
test('Add a product to the cart with quantity = 1.5', async ({ page }) => {
  await page.goto('http://localhost/index.php');
  await page.getByRole('link', { name: ' My Account ' }).click();
  await page.getByRole('link', { name: 'Login' }).click();
  await page.getByRole('textbox', { name: 'E-Mail Address' }).click();
